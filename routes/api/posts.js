...
async function UpdateServerData() {
    ...
    // Users table'
    let usersData = await UsersCollection.find({}, {projection: {userMoney: 1, id: 1, userFaction: 1, userIdentity: 1, username: 1, adminLvl:1, playerHours: 1, vipLvl: 1, userBans: 1, lastLogin: 1}}).toArray();
    for (const user of usersData) {
        ...
        if (user.adminLvl > 0) {
            StaffMembers.push({
                ...
                lastLogin: mongodb.GetLuaTimestamp(user.lastLogin),
                hoursPlayed: user.playerHours,
            })
        }
        if (user.userBans) {
            BannedUsers.push({
    ...
    let uData = await UsersCollection.find({}).toArray();
    for (const data of uData) {
        ...
        for (const [key, item] of Object.entries(inventoryData)) {
            UsersData[data.id].inventory.push({
                name: key,
                amount: item.amount,
            })    
        }
